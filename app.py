...
if st.sidebar.button("🚀 Run Analysis", type="primary"):
    ...
    try:
        ...
        with col1:
            ...
            st.metric("Total Return", f"₹{total_return}", f"{return_pct}%")
        
        with col2:
            xirr_value_rounded = (xirr_value * 100)
            st.metric("XIRR (Annualized)", f"{xirr_value_rounded}%")
        
        with col3:
            st.metric("Total Trades", len(portfolio['history']))
        
        with col4:
            st.metric("Final Value", f"₹{portfolio['cash']}")
        ...
        with comp_col1:
            st.metric("Buy & Hold Return", f"{buy_hold_return}%")
        
        with comp_col2:
            st.metric("Buy & Hold Annualized", f"{buy_hold_annualized}%")
        
        with comp_col3:
            outperformance = xirr_value * 100 - buy_hold_annualized
            st.metric("Strategy Outperformance", f"{outperformance}%")
